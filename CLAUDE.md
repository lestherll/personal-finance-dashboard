...
- **Three-tier matching engine** (`transformers/matching.py`): same-source dedup by full content fingerprint (account, date, amount_minor, normalized description, occurrence); a same-source **overlap-window** tier collapses that fingerprint across two ingestions of the same source_type/account whose *declared* `statement_period_from/to` overlap (e.g. a partial-month statement later re-covered by a full-month statement) — restricted to the overlap window, multiset-matched so genuine same-day repeats and any excess on either side survive untouched; declared cross-source pairs (Natwest Transactions+Statement) match on loose key (account, date, amount_minor) with multiset counting, preferring the statement. `silver_transaction_id` is minted as `source_type:fingerprint_occurrence` (source-scoped so two sources with identical content can't collide). There is deliberately **no** bank-transaction-id tier — the only source with a genuine bank id is the effectively-unused Monzo CSV export; see the matching.py docstring for why a synthesised id wouldn't earn top rank
...
3. **Re-upload deduplication:** Content-addressed by SHA-256. Same file → same `ingestion_id` → manifest already complete → skip. Different files with the same original filename → two separate ingestion_ids, no collision. Cross-file transaction matching is handled by the three-tier matching engine (`transformers/matching.py`), including overlap-window collapsing when a broader statement later re-covers a period an earlier, narrower statement already reported.
