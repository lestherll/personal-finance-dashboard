...
- `account_ledger` now covers Natwest CSV, Vanguard CSV, Kroo, AmEx, First Direct, and Natwest Statement. Still excluded: `natwest-transactions` (the online export has no balance data in the source document at all) and `vanguard-pdf` (its per-line `cash_balance` is a different metric from Portfolio Value — kept in `raw_data` but not wired into the ledger). See Gotcha #6.
